...
uint256 hashGenesisBlock("0x658c2f6da7071561eb703ad4b0cebdd13bd74e8b4f08782b5bd74689573b6a87");
...
const string strMessageMagic = "BlockCoin Signed Message:\n";
...
{
    // BlockCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
	...
	static const int64	BlocksTargetSpacing			= 7 * 60;
	unsigned int		TimeDaySeconds				= 60 * 60 * 24;
	int64				PastSecondsMin				= TimeDaySeconds * 7 * 0.1;
	int64				PastSecondsMax				= TimeDaySeconds * 7 * 2.8;
...
{
    // BlockCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "blockcoin genesis block";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
...
unsigned char pchMessageStart[4] = { 0xc4, 0xcc, 0xaf, 0xeb };
...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn)
{
    ...
    // until there are no more or the eaches this size:
...
void static BlockCoinMiner(CWallet *pwallet)
{
    ...
    try { loop {
        ...
        printf("Running BlockCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BlockCoinMiner, pwallet));
}
