...
// Copyright (c) 2011-2012 BlockCoin Developers
...
void ThreadIRCSeed()
{
    ...
    RenameThread("BlockCoin-ircseed");
...
void ThreadIRCSeed2()
{
    ...
    while (true)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #BlockCoinTEST0\r");
            Send(hSocket, "WHO #BlockCoinTEST0\r");
        } else {
            // randomly join #BlockCoin00-#BlockCoin99
            // network is now over 3k peers , get them to join 50 random channels!
            //            channel_number = 0; 
            int channel_number = 0; //GetRandInt(50);

            Send(hSocket, strprintf("JOIN #BlockCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #BlockCoin%02d\r", channel_number).c_str());
        }
